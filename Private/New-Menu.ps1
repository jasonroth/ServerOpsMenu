...
$hash = [ordered]@{
    ...
    'Restart Service' = {Get-Service (Read-Host 'Service Name') | Restart-Service -Force -Verbose}
    ...
    'Stop Process' = {Get-Process (Read-Host 'Process Name') | Stop-Process -Force -Verbose}
    'Get Local Volumes' = {Get-CimInstance Win32_LogicalDisk | Where-Object DriveType -eq 3 | foreach {[pscustomobject]@{ID = $_.DeviceID;VolumeName = $_.VolumeName;'Disk Space' = '{0}GB/{1}GB Used' -f (([math]::round($_.Size/1GB)-[math]::round($_.FreeSpace/1GB))),([math]::round($_.Size/1GB))}}}
    'Get Recent System Eventlog' = {Get-Eventlog -LogName System -Newest 50 | Select-Object Index, Time, InstanceID, Message | Format-Table -AutoSize}
